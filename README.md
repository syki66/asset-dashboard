...
6. **일자역순**으로 정렬한 뒤, 화면에서 우클릭 후 **엑셀로 내보내기 (CSV)**를 선택합니다.
...
- **백그라운드 프라이버시 스크린**  
  앱이 백그라운드로 전환되면 프라이버시 스크린을 표시해 자산 정보를 가립니다. WebAuthn 기반 기기 인증을 활성화하면 지원 기기에서 Face ID, Touch ID, Windows Hello 또는 기기 PIN으로 화면 잠금을 해제할 수 있습니다.
- **Toast 알림 제공**  
...
Setup 진입점은 일반 화면 `/setup`, 체험 화면 `/setup?mode=demo`, 암호화 CSV 관리 화면 `/admin`으로 구분합니다. `/admin`은 로그인하지 않은 사용자를 먼저 `/login`으로 보내며, 로그인 후 원래 관리 화면으로 돌아옵니다.

Supabase Auth 기반 로그인·회원가입·세션 관리와 사용자별 암호화 CSV 저장 기능 자체는 구현되어 있습니다. 다만 현재는 `/admin`으로 진입한 경우에만 로그인을 강제하고 암호화 CSV 관리 UI를 표시하며, Supabase Auth 설정에서도 신규 회원가입을 비활성화한 상태입니다.

이 제한은 기능 개발이 완료되지 않아서가 아니라, 공개 운영 전 임의 계정 생성과 불필요한 Auth 사용자·DB 트래픽·저장공간 사용을 방지하기 위한 것입니다. Supabase Auth에서 신규 회원가입을 활성화하고 `/admin` 접근 정책을 조정하면 별도의 백엔드 추가 구현 없이 로그인부터 암호화 CSV 저장·불러오기까지 정상적으로 사용할 수 있습니다.
...
- [ ] 운영 연락처를 도메인 기반 이메일로 변경 (푸터와 SEO 구조화 데이터 포함)
- [ ] Setup 페이지의 전체 설정값을 Supabase에 저장하고 다시 접속했을 때 불러오도록 구현
