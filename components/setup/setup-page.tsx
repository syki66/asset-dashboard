...
export default function Page({ isAdmin = false }: SetupPageProps = {}) {
  return (
    <Suspense fallback={<SetupPageFallback />}>
      <SetupPageContent isAdmin={isAdmin} />
    </Suspense>
  );
}

function SetupPageContent({ isAdmin = false }: SetupPageProps) {
  ...
  const adminReturnPath = `${pathname}${currentQuery ? `?${currentQuery}` : ''}`;
  const adminLoginPath = `/login?next=${encodeURIComponent(adminReturnPath)}`;
